...
// TestProviderReq 测试供应商连接请求
type TestProviderReq struct {
        BaseURL string `json:"base_url" binding:"required,url,max=500"` // 供应商Base URL
        APIKey  string `json:"api_key" binding:"required,max=500"`    // 供应商API Key（明文或脱敏值）
        ID      *uint  `json:"id"`                                    // 编辑模式下的供应商ID，用于从后端获取真实API Key
}
        ...
        api := router.Group("/api")
        {
                ...
                auth.Use(middleware.AuthRequired(s.jwtSecret))
                {
                        ...
                        admin.Use(middleware.AdminRequired(s.jwtSecret))
                        {
                                ...
                                admin.POST("/providers/test", s.handleTestProvider)
...
// 使用供应商的baseURL和API-Key访问其OpenAI兼容接口/v1/models来验证配置是否正确
// 在后端实现是因为：1.避免前端跨域问题 2.前端只有脱敏的API Key，无法用于测试
// 支持两种模式：
//   - 新建模式：前端传入明文 api_key，直接使用
//   - 编辑模式：前端传入脱敏 api_key（包含 "****"），后端根据 id 从数据库解密真实密钥
func (s *Server) handleTestProvider(c *gin.Context) {
        ...
        // 如果API Key包含脱敏标记 "****"，说明前端传的是脱敏值，需要从数据库获取真实密钥
        if strings.Contains(apiKey, "****") {
                if req.ID == nil {
                        c.JSON(400, gin.H{"success": false, "error": "API Key为脱敏值，请提供供应商ID或重新输入API Key"})
                        return
                }

                // 从数据库查找供应商并解密API Key
                var provider database.Provider
                if err := s.db.First(&provider, *req.ID).Error; err != nil {
                        c.JSON(404, gin.H{"success": false, "error": "供应商不存在，无法获取API Key"})
                        return
                }

                decryptedKey, err := database.DecryptAPIKey(provider.APIKey, s.encryptionKey)
                if err != nil {
                        log.Printf("[管理] 测试供应商时解密API Key失败: %v (provider_id=%d)", err, *req.ID)
        ...
        // 构建目标URL：{base_url}/models
        // BaseURL约定：包含完整的API基础路径含版本号，例如：
        //   - https://api.openai.com/v1 → 目标: /v1/models
        //   - https://open.bigmodel.cn/api/paas/v4 → 目标: /v4/models
        //   - https://my-server.com → 目标: /v1/models (无版本路径时追加/v1)
        baseURL := strings.TrimRight(req.BaseURL, "/")
        var targetURL string
        if versionPathSuffix.MatchString(baseURL) {
                // BaseURL已含版本路径，直接追加 /models
                targetURL = baseURL + "/models"
        } else {
                // BaseURL不含版本路径，追加 /v1/models
        ...
        // 设置认证头（使用解密后的真实apiKey，而非脱敏值）
        httpReq.Header.Set("Authorization", "Bearer "+apiKey)
        httpReq.Header.Set("Accept", "application/json")

        // 创建HTTP客户端（跳过TLS验证以支持自签证书的私有部署）
        // 显式配置HTTP/2支持，避免与强制HTTP/2的供应商（如CDN）协商失败
        ...
        log.Printf("[管理] 供应商测试成功: url=%s, 模型数量=%d, 延迟=%dms", targetURL, len(modelIDs), latency)
